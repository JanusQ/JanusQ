...
class Iteration():
    def __init__(self, n_qubits, threshold=1e-10):
        self.n_qubits = n_qubits
    ...
    def mitigate(self, statscnt: tuple[np.ndarray, np.ndarray], measured_qubits: list = None):
...
class Mitigator():
    def __init__(self, n_qubits, n_iters=2, threshold=1e-5):
        self.n_qubits = n_qubits
    ...
    def mitigate(self, statscnt: dict, measured_qubits=None):
        if isinstance(statscnt, dict):
            statscnt = statuscnt_to_npformat(statscnt)
        ...
        for iter in self.iters:
            opt_statscnt = iter.mitigate(
                opt_statscnt, measured_qubits=measured_qubits)

...
from data_objects.circuit import Circuit, qiskit_to_circuit
...
def get_algs(n_qubits, backend: Backend, algs = ['qft', 'hs', 'ising', 'qknn', 'qsvm', 'vqc', 'ghz', 'grover'], unitary = False) -> list[Circuit]:
    ...
    if 'qft' in algs:
        circuits.append(get_data(f'qft', qft.get_circuit(n_qubits), **kwargs))
    if 'hs' in algs:    
        circuits.append(get_data(f'hs', hamiltonian_simulation.get_circuit(n_qubits), **kwargs))
    if 'ising' in algs:    
        circuits.append(get_data(f'ising', ising.get_circuit(n_qubits), **kwargs))
    if 'qknn' in algs:
        circuits.append(get_data(f'qknn', qknn.get_circuit(n_qubits), **kwargs))
    if 'qsvm' in algs:
        circuits.append(get_data(f'qsvm', qsvm.get_circuit(n_qubits), **kwargs))
    if 'vqc' in algs:
        circuits.append(get_data(f'vqc', vqc.get_circuit(n_qubits), **kwargs))
    if 'ghz' in algs:
        circuits.append(get_data(f'ghz', ghz.get_circuit(n_qubits), **kwargs))
    if 'grover' in algs:
        circuits.append(get_data(f'grover', grover.get_circuit(n_qubits), **kwargs))
...
def get_data(id, qiskit_circuit: QuantumCircuit, mirror, backend: Backend, should_transpile=True):
    new_qiskit_circuit = QuantumCircuit(qiskit_circuit.num_qubits)
    for instruction in qiskit_circuit:
        if instruction.operation.name in ('id',):
            continue
...
def get_algs(n_qubits, backend: Backend, algs = ['qft', 'hs', 'ising', 'qknn', 'qsvm', 'vqc', 'ghz', 'grover']) -> list[Circuit]:
    circuits = []
    mirror = False

...
from data_objects.circuit import Circuit, Gate, Layer, SeperatableCircuit, circuit_to_qiskit, qiskit_to_circuit
from data_objects.random_circuit import random_1q_layer
                ...
                if n_error_paths > 0:
                    for qubit in gate['qubits']:
                        error_layer.append(Gate({
                            'qubits': [qubit],
                            'params': [(random.random() / 20 - 1 / 10) * pi * n_error_paths],
                            'name': 'rx'
                        }))

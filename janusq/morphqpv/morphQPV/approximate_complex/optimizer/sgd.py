...
def sgd_output_cost_function(parms,inputs,outputs,real_state):
    ...
    if len(buildinput.shape)>1:
        ...
        return  jnp.mean(jnp.abs(buildoutput-real_state))
    # + 10*(jnp.abs(buildinput @ buildinput.conj().T - 1)+ jnp.abs(buildoutput @ buildoutput.conj().T - 1))
...
class SgdOptimizer:
    ...
    def optimize_params(self,inputs,outputs,real_target,target='input'):
        ...
        return:
            bast_params: 优化后的参数
        """
        parms= random.normal(random.PRNGKey(np.random.randint(0,1000)), (len(inputs),)) + 1j*random.normal(random.PRNGKey(np.random.randint(0,1000)), (len(inputs),))
        min_iter = 0

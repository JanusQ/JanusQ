...
def decompose(inputs,outputs,real_target,target='input',optimizer='linear_program',steps=1000,learning_rate=0.001,jobname=None):
    ...
    Args:
        ...
        optimizer: 优化器，'linear_program'或者'sgd'
        steps: 迭代次数
    Returns:
        best_params: 最优参数
    """
...
def infer_process_by_density(process:list,real_density,output_qubits,method,base_num,target='input',input_qubits=None,optimizer='linear_program',learning_rate=0.001,steps=5000):
    """ 通过密度矩阵构建量子态
    Args:
        process: 量子线路
    ...
    inputs = []
    outputs = []
    inputs,outputs = process_sample(process,input_qubits,base_num=base_num,method=method,output_qubits=output_qubits)
...
def infer_process_by_statevector(process:list,real_state,method,base_num,input_qubits=None,output_qubits=None,target='input',optimizer='linear_program',learning_rate=0.01,steps=5000,jobname='linear_program',device='ibmq'):
    """ 通过量子状态向量构建量子态
    Args:
        process: 量子线路
        real_state: 真实的量子态
...
def infer_process_statistical(process:list,real_state,method,base_num,input_qubits=None,output_qubits=None,target='input',optimizer='linear_program',learning_rate=0.01,steps=5000,jobname='linear_program',device='ibmq'):
    """ 通过量子状态向量构建量子态
    Args:
        process: 量子线路
        real_state: 真实的量子态
...
def unit_test():
    ...
    build_input,build_output = infer_process_statistical(layer_circuit[1:],input_statevector,optimizer='sgd',method='random',base_num=2**3,target='input')
